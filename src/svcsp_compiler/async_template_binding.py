...
def bind_async_templates(architecture: AsyncMicroarchitecture) -> BoundAsyncModule:
    ...
    for index, input_port in enumerate(join.inputs):
        ...
        if input_port.en_receive is not None:
            adapter = input_port.en_receive
            adapter_id = f"en_receive_{index}"
            instances.append(BoundAsyncInstance(adapter_id, "en_receive", adapter))
            enable = signal(f"{adapter_id}_enable", "enable", behavioral.ONE_BIT)
            connections.append(BoundAsyncConnection(enable, f"{adapter_id}.enable", "enable"))
        connections.extend((
    ...
    for index, output in enumerate(fork.outputs):
        ...
        adapter_id = instance_id
        if output.en_send is not None:
            adapter = output.en_send
            adapter_id = f"en_send_{index}"
            instances.append(BoundAsyncInstance(adapter_id, "en_send", adapter))
            enable = signal(f"{adapter_id}_enable", "enable", behavioral.ONE_BIT)
            connections.append(BoundAsyncConnection(enable, f"{adapter_id}.enable", "enable"))
        connections.extend((
    ...
    instances, signals, connections, port_bindings, parameter_bindings = _typed_bindings(
    ...
    return BoundAsyncModule(architecture, tuple(instances), tuple(signals), tuple(ports), tuple(connections),
                            tuple(port_bindings), tuple(parameter_bindings))
...
def _typed_bindings(architecture: AsyncMicroarchitecture, instances: list[BoundAsyncInstance],
    ...
    for index, input_port in enumerate(join.inputs):
        ...
        if input_port.en_receive is not None:
            adapter = input_port.en_receive
            adapter_instance = by_source[adapter]
            enable = ensure(f"en_receive_{index}_enable", "enable", behavioral.ONE_BIT)
            adapter_formals = ("external_req", "external_ack", "external_data", "body_req", "body_ack", "body_data", "enable")
            formals[adapter_instance.id] = adapter_formals
            for formal, actual in (
                ("external_req", external(operation.channel, "receive", "request")),
                ("external_ack", external(operation.channel, "receive", "acknowledge")),
                ("external_data", external(operation.channel, "receive", "payload")),
                ("body_req", req), ("body_ack", ack), ("body_data", data), ("enable", enable),
            ):
                bind(adapter_instance, formal, actual)
    ...
    for index, output in enumerate(fork.outputs):
        ...
        if output.en_send is not None:
            ...
            enable = ensure(f"en_send_{index}_enable", "enable", behavioral.ONE_BIT)
            formals[adapter_instance.id] = ("body_req", "body_ack", "body_data", "external_req", "external_ack", "external_data", "enable")
            for formal, actual in (
                ("body_req", body_req), ("body_ack", body_ack),
                ("body_data", f"storage_{architecture.storage.slots.index(slot)}_data"),
                ("external_req", external(operation.channel, "send", "request")),
                ("external_ack", external(operation.channel, "send", "acknowledge")),
                ("external_data", external(operation.channel, "send", "payload")), ("enable", enable),
            ):
                bind(adapter_instance, formal, actual)
